...
const init = () => {
 ...
 const listeImg = dataImages.map(data => createImage(data));

  // ajout des événements + insertion dans le mur
...
*/
const filtreImages = event => {  const texte = event.target.value.toLowerCase();

 const images = document.querySelectorAll("#mur img");

 images.forEach(img => {
 const titre = img.title.toLowerCase();

 if (titre.includes(texte)) {
